...
import { Search, CheckCircle, AlertCircle, Edit2, Trash2, Plus, X, DollarSign, AlertTriangle, Home, MapPin, RefreshCcw } from 'lucide-react';
import {
  formatCurrency,
  formatDate,
  DUES_STRUCTURE,
        ...
        <div className="overflow-y-auto max-h-[90vh]">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="bg-slate-50 rounded-xl p-4 space-y-4">
              <h3 className="font-semibold text-slate-900 text-sm uppercase tracking-wide">Member Information</h3>
                ...
                <div className="bg-rose-50 border-2 border-rose-300 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-rose-900 mb-2">Bylaw Compliance Warning</p>
                      ...
                      <label className="flex items-center gap-2 mb-3">
                        ...
                        />
                        <span className="text-sm font-medium text-rose-900">Override bylaw limit (requires approval)</span>
                      </label>

                      {bylawOverride.enabled && (
                        <div className="space-y-3">
                          <div>
                            <label className="block text-xs font-medium text-rose-900 mb-1">Override Reason *</label>
                            <textarea
                              value={bylawOverride.reason}
                              onChange={(e) => setBylawOverride({ ...bylawOverride, reason: e.target.value })}
                              className="w-full px-3 py-2 text-sm border border-rose-300 rounded-lg focus:ring-2 focus:ring-rose-500"
                              rows="2"
                              placeholder="e.g., Board approved exception for immediate family member"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-medium text-rose-900 mb-1">Approved By *</label>
                            <input
                              type="text"
            ...
            <div className="bg-blue-50 rounded-xl p-4 space-y-4">
              <h3 className="font-semibold text-slate-900 text-sm uppercase tracking-wide">Dues Calculation</h3>

              <div className="bg-white rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm text-slate-600">Base Dues:</span>
                  <span className="text-lg font-bold text-slate-900">{formatCurrency(formData.dues.baseDues)}</span>
                </div>

                <div className="border-t border-slate-200 pt-3 mb-3">
                  <p className="text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Additional Fees:</p>
                  {formData.dues.additionalFees.map(fee => (
                    <label key={fee.type} className="flex items-center justify-between py-2 cursor-pointer hover:bg-slate-50 rounded px-2">
                      ...
                      <span className="text-sm font-semibold text-emerald-600">+{formatCurrency(fee.amount)}</span>
                    </label>
                  ))}
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 mt-2">
                    <span className="text-sm text-slate-600">Subtotal with Fees:</span>
                    <span className="text-lg font-bold text-slate-900">{formatCurrency(calculatedDues.totalValue)}</span>
                  </div>
                </div>

                <div className="border-t border-slate-200 pt-3">
                  <p className="text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wide">Discounts:</p>
                  {formData.dues.discounts.map(discount => (
                    <div key={discount.type} className="mb-3">
                      <label className="flex items-center justify-between py-2 cursor-pointer hover:bg-slate-50 rounded px-2">
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            ...
                            className="w-4 h-4 text-rose-600 rounded focus:ring-rose-500"
                          />
                          <span className="text-sm text-slate-700">{discount.label}</span>
                        </div>
                        <span className="text-sm font-semibold text-rose-600">{formatCurrency(discount.amount)}</span>
                      </label>
                  ...
                  <div className="mb-3 border-t border-slate-200 pt-3">
                    <button
                      ...
                      className={`w-full flex items-center justify-between py-2 px-2 rounded hover:bg-slate-50 transition-colors ${
                        showOtherDiscount ? 'bg-rose-50' : ''
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <input
                        ...
                        />
                        <span className="text-sm text-slate-700 font-medium">Other Discount</span>
                      </div>
                      <span className="text-sm text-slate-500 italic">Custom amount</span>
                    </button>
                    {showOtherDiscount && (
                      <div className="ml-6 mt-2 space-y-2">
                        <div>
                          <label className="block text-xs font-medium text-slate-600 mb-1">Discount Amount * (enter positive number)</label>
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) => setOtherDiscountAmount(e.target.value)}
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 mt-2">
                    ...
                    <span className="text-lg font-bold text-rose-600">{formatCurrency(calculatedDues.totalDiscounts)}</span>
                  </div>
                </div>

                <div className="border-t-2 border-slate-300 pt-3 mt-3">
                  <div className="flex items-center justify-between">
                    <span className="text-base font-bold text-slate-900 uppercase">Total Realized Revenue:</span>
                    <span className="text-2xl font-bold text-blue-600">{formatCurrency(calculatedDues.totalRealized)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Payment Information */}
            <div className="bg-slate-50 rounded-xl p-4 space-y-4">
              <h3 className="font-semibold text-slate-900 text-sm uppercase tracking-wide">Payment Information (Optional)</h3>
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">Member has paid</span>
              </label>
            ...
            {(formData.hasPaid || (isEditing && member && member.datePaid)) && !(isEditing && member && member.refunded) && (
              <div className="bg-rose-50 rounded-xl p-4 space-y-4 border-2 border-rose-200">
                <h3 className="font-semibold text-rose-900 text-sm uppercase tracking-wide">Process Refund</h3>
                ...
                <label className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4 text-rose-600 rounded focus:ring-rose-500"
                  />
                  <span className="text-sm font-medium text-rose-900">Issue refund for membership dues</span>
                </label>
                ...
                {formData.refunded && (
                  <div className="space-y-3">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-rose-900 mb-2">Refund Amount *</label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-700 text-lg">$</span>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
                          ...
                          className="w-full pl-8 pr-4 py-3 border border-rose-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-lg font-semibold"
                          placeholder="0.00"
                          required
                        />
                      </div>
                      <p className="text-xs text-rose-700 mt-1">
                        ...
                        {formData.refundAmount > 0 && formData.refundAmount < (formData.dues?.totalRealized || calculatedDues.totalRealized) && (
                          <span className="ml-2 font-semibold text-emerald-700">→ PARTIAL REFUND (Member stays Paid)</span>
                        )}
                        {formData.refundAmount >= (formData.dues?.totalRealized || calculatedDues.totalRealized) && formData.refundAmount > 0 && (
                          <span className="ml-2 font-semibold text-amber-700">→ FULL REFUND (Member marked Refunded)</span>
                        )}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-rose-900 mb-2">Refund Date *</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-rose-900 mb-2">Refund Reason *</label>
                      <textarea
            ...
            {isEditing && member && member.refunded && (
              <div className="bg-rose-100 rounded-xl p-4 border-2 border-rose-300">
                <h3 className="font-semibold text-rose-900 text-sm uppercase tracking-wide mb-2">Refund Previously Issued</h3>
                ...
                <p className="text-xs text-rose-700 mt-2 italic">This member has already been refunded. Revenue and OPEX expense transactions are in the system.</p>
              </div>
            )}
...
function Members({ data, onRefresh, fiscalYear }) {
  ...
  // Ensure all members have residence field (migration for old data)
  ...
  const filteredMembers = membersWithResidence.filter(m => {
  ...
  const memberMetrics = calculateMemberMetrics(membersWithResidence);
  const compliance = memberMetrics.compliance;

  const stats = {
    total: membersWithResidence.length,
    paid: membersWithResidence.filter(m => {
      if (!m.datePaid && !m.refunded) return false; // Unpaid, no refund
      if (!m.refunded) return m.datePaid; // Paid, no refund
      // Has refund - check if partial (still counts as paid) or full
      const memberDues = m.dues?.totalRealized || m.totalRealized || 0;
      return m.refundAmount < memberDues; // Partial refund = still paid
    }).length,
    unpaid: membersWithResidence.filter(m => !m.datePaid && !m.refunded).length,
    refunded: membersWithResidence.filter(m => {
      if (!m.refunded) return false;
      const memberDues = m.dues?.totalRealized || m.totalRealized || 0;
      return m.refundAmount >= memberDues; // Only full refunds count as "refunded"
    }).length,
    totalRevenue: membersWithResidence.reduce((sum, m) => sum + (m.dues?.totalRealized || m.totalRealized || 0), 0),
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
          <p className="text-sm text-slate-600 mb-1">Refunded</p>
          <p className="text-2xl font-bold text-amber-600">{stats.refunded}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
          <p className="text-sm text-slate-600 mb-1">Inside / Outside</p>
          <p className="text-xl font-bold">
            <span className="text-emerald-600">{stats.inside}</span>
            <span className="text-slate-400 mx-1">/</span>
            <span className="text-blue-600">{stats.outside}</span>
          </p>
          <p className="text-xs text-slate-500 mt-1">
            {stats.outsidePercentage}% outside
            {!stats.compliant && <span className="text-rose-600"> ⚠️</span>}
          </p>
        </div>
      </div>
            ...
            {['all', 'paid', 'unpaid', 'refunded'].map(status => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  filterStatus === status
                    ? (status === 'refunded' ? 'bg-amber-600 text-white' : 'bg-blue-600 text-white')
      ...
      <button
        ...
        className="fixed bottom-8 right-28 w-14 h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center z-40 group"
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Member Name</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Date Paid</th>
              ...
              {filteredMembers.length === 0 ? (
                ...
                filteredMembers.map(m => {
                  ...
                  <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">{m.type || m.newReturn || 'Return'}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDate(m.datePaid)}</td>
